...
namespace Avva.MotionFramework
{
    public class YaskawaMotion: IAvvaMotion, IDisposable
    {
		...
		{
			BaseMoveTo(axis, distance, WaitForCompletion,velocity,isRelative);
		}
        ...
        public void AsyncMoveTo(int[] axis, double[] distance, bool isRelative)
        {
            BaseMoveTo(axis, distance, WaitForStart, isRelative);
        }
		...
		private void BaseMoveTo(int axis, double distance, UInt16[] waitType,  double velocity, bool isRelative = true)
		{
			CMotionAPI.POSITION_DATA[] move_pos = (CMotionAPI.POSITION_DATA[])PosForMove.Clone();
